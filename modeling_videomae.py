    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            if self.config.num_channels != 3:
                ...
                mean = torch.as_tensor(IMAGE_MEAN).to(device=device, dtype=dtype)[
                    None, None, :, None, None
                ]
                std = torch.as_tensor(IMAGE_STD).to(device=device, dtype=dtype)[
                    None, None, :, None, None
                ]
        ...
        # Fixes https://github.com/huggingface/transformers/issues/27295
